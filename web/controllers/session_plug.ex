defmodule Carbon.SessionPlug do
  import Plug.Conn
  alias Carbon.{SessionController, User}
  ...
  def call(conn, repo) do
    conn
    |> maybe_assign_user_from_session(repo)
    |> maybe_assign_user_from_token(repo)
    |> maybe_redirect_to_login_page
  end

  defp maybe_assign_user_from_session(conn, repo) do
    user_id = get_session(conn, :current_user_id)
    user = user_id && repo.get(User, user_id)
    assign(conn, :current_user, user)
  end

  defp maybe_assign_user_from_token(%Plug.Conn{assigns: %{current_user: user}} = conn, _repo) when user != nil, do: conn
  defp maybe_assign_user_from_token(conn, repo) do
